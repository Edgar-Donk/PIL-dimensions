"""Antialiased Dimension Toolbox for PIL using attr module, otherwise very
similar to DimLinesAA, except that some function attributes are stored in the
attr
...
dimension_attr.py line with arrows used as base for other dimensions
 ...
 dimension_attr normal dimensioning, used as base for other dimensions
 arc_dim_attr dimension over an angle
...
class atr:
    """attr is a repository of infrequently changed function attributes
    Parameters
    ----------
    width : int
        size of PIL image, validated
    height : int
        size of PIL image, validated
    image : str
        PIL image handle, made after initialisation
    dr : str
        PIL drawing handle, made after initialisation
    fill : tuple[int,int,int]
        rgb line colour tuple, default (0,0,0), validated
    back : tuple[int,int,int]
        rgb background colour tuple, default light straw, validated
    arrowhead : tuple[int,int,int]
        tuple arrow size and shape, default (8, 10, 3), validated
    """
...
def WideLineATTR(ptA, ptB, width=1):
    ...
    for i in range(int(ed)+1):
        if aq.fill == (0,0,0):
            ...
            diffs[i] = tuple(errs(aq.fill[j],ed,i) for j in range(3))
...
def PartCircleATTR(xm, ym, r, start, finish, sects, width=1):
    ...
    for i in range(maxd):
        if aq.fill == (0,0,0):
            ...
            diffs[i] = tuple(errs(aq.fill[k],maxd,i) for k in range(3))
    ...
    for i in range(maxdsm):
        if aq.fill == (0,0,0):
            ...
            diffsm[i] = tuple(errs(aq.fill[k],maxdsm,i) for k in range(3))
...
def arc_dim_attr(centre,radius,begin,end,text=None):
    ...
    if text is None:
        text = str(diff) + '°'
    (width, height) = aq.font.getsize(text)
    ...
    angled_text(aq.image, (X, Y), text, angle, aq.font, aall=0)
...
def slant_dim_attr(ptA, ptB =None, extA=None,  angle=None, length=None, text=None):
    ...
    LineATTR((int_up(ptC[0]+3*sin(phir)), int_up(ptC[1]-3*cos(phir))),
            (int_up(ptC[0]-(extO-1)*sin(phir)), int_up(ptC[1]+(extO-1)*cos(phir))))
    LineATTR((int_up(ptD[0]+3*sin(phir)), int_up(ptD[1]-3*cos(phir))),
    ...
    angled_text(aq.image, at, text, angle, aq.font)
...
def level_dim_attr(at, diam, ext=0, ldrA=20, ldrB=20, dash=(10,4), text=None, tri=8):
    ...
    if ldr > 0:
        if ext == 0:
            DashedLineATTR(aq.draw, p2, p4, dash=dash, fill=aq.fill, back=aq.back)
        else:
            LineATTR(aq.draw, p2, p4, fill=aq.fill, back=aq.back)
            aq.draw.line([p7, (p7[0] + exto, p7[1])], width = 1, fill=aq.fill, back=aq.back)
    else:
        if ext ==0:
            DashedLineATTR(aq.draw, p1, p4, dash=dash, fill=aq.fill, back=aq.back)
        else:
            LineATTR(aq.draw, p1, p4, fill=aq.fill, back=aq.back)
            aq.draw.line([p7, (p7[0] - exto, p7[1])], width = 1, fill=aq.fill, back=aq.back)
