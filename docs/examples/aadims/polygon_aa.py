...
def LineAA(draw, pta, ptb, fill=(0,0,0), back=(255,255,255),cross=0):
    ...
    for x in range (dr): # x0, x1+1     # pixel loop
        draw.point([x0, y0], fill=diffs[abs(err-dx+dy)] if abs(err-dx+dy) < ed//2 else int_up(ed))
...
def centroid(points):
    # assume that points is a 2D list of points polygon, count tip twice
    points = points+[points[0],]
    x = [p[0] for p in points]
    y = [p[1] for p in points]
    centroid = int_up(sum(x) / len(points)), int_up(sum(y) / len(points))
    #centroid = int_up((sum(x)+points[0][0]) / len(points)+1), \
            #int_up((sum(y)+points[0][1]) / len(points)+1)
    print(points[0][0],points[0][1], 'tip',x,y)
    return centroid
...
if __name__ == "__main__":
    ...
    pts = [(126, 85), (128, 96), (130, 92), (134, 93)] # [(5,5),(7,16),(9,12),(13,13)] #
    #[10,5,13,15,10,13,7,15] # [10,15,13,5,10,7,7,5] #
